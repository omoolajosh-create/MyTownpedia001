...
-- Towns policies
create policy if not exists "Towns are viewable by everyone"
  on public.towns for select using (true);

create policy if not exists "Admins can manage towns"
  on public.towns for all using (public.has_role(auth.uid(), 'admin')) with check (public.has_role(auth.uid(), 'admin'));

create trigger if not exists trg_towns_updated_at
before update on public.towns
for each row execute function public.update_updated_at_column();

-- 5) Stories
create type if not exists public.story_type as enum ('personal','historical','cultural','legend');
...
create policy if not exists "Anyone can read approved comments"
  on public.comments for select using (is_approved = true);
-- Authors can read their own comments
create policy if not exists "Authors can read their comments"
...
create policy if not exists "Admins can manage comments"
...
-- 7) Media gallery
create type if not exists public.media_type as enum ('image','video','audio');
...
create policy if not exists "Media is viewable by everyone"
...
create policy if not exists "Admins can manage all media"
  on public.media_gallery for all using (public.has_role(auth.uid(), 'admin')) with check (public.has_role(auth.uid(), 'admin'));

-- 8) Notifications
create type if not exists public.notification_type as enum ('general','story_approved','comment','admin');
...
create policy if not exists "User can see own notifications"
  on public.notifications for select using (auth.uid() = user_id);
create policy if not exists "User can create own notifications"
  on public.notifications for insert with check (auth.uid() = user_id);
create policy if not exists "Admins can manage notifications"
